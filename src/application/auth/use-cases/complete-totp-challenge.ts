...
export class CompleteTotpChallenge {
  ...
  async execute(input: {
    ...
    if (factor === null || factor.status !== 'ENABLED') throw new NotFoundError();
      ...
      const rate = await this.dependencies.transaction.execute(
        async ({ challenges, rateLimits }) => {
          ...
          return rateLimits.recordFailure({
            ...
            now: at,
            windowSeconds: this.dependencies.policy.rateLimitWindowSeconds,
            lockSeconds: this.dependencies.policy.rateLimitLockSeconds,
            maximumFailures: this.dependencies.policy.rateLimitMaxFailures,
          });
        },
      );
    ...
    return this.dependencies.transaction.execute(async (repositories) => {
      if (!(await repositories.totpFactors.acceptCounter(factor.publicId, counter, at))) {
        throw new AuthenticationError();
      }
