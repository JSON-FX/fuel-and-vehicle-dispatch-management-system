export interface FuelApiEnvelope<T> {
...
export async function readFuelApiResponse<T>(response: Response): Promise<T> {
  const envelope = (await response.json()) as FuelApiEnvelope<T>;
