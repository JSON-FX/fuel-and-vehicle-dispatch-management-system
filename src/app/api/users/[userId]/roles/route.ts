...
import { publicIdSchema, roleIdsSchema } from '@/lib/auth/route-schemas';
...
export async function PUT(request: Request, context: Context): Promise<Response> {
  ...
  return withResponseHandler(composition, async ({ request: currentRequest, requestId }) => {
    ...
    const { rolePublicIds } = roleIdsSchema.parse(await parseJsonBody(currentRequest));
    await composition.assignUserRoles.execute({
