...
export async function GET(request: Request, route: Context): Promise<Response> {
  ...
  return withResponseHandler(composition, async ({ request: currentRequest, requestId }) => {
    const authenticated = await authenticateBudgetRequest(
      ...
      'read',
      requestId,
      '/api/budget-allocations/:budgetAllocationId',
    );
    return composition.getBudgetAllocation.execute({
...
export async function PATCH(request: Request, route: Context): Promise<Response> {
  ...
  return withResponseHandler(composition, async ({ request: currentRequest, requestId }) => {
    ...
    return composition.updateBudgetAllocation.execute({
      context: budgetRequestContext(currentRequest, authenticated.principal, requestId),
      publicId: budgetAllocationPublicIdSchema.parse((await route.params).budgetAllocationId),
      command: patchBudgetAllocationSchema.parse(await parseJsonBody(currentRequest)),
    });
  })(request);
}
